...
/** Rozklikávací tlačítko jedné skupiny důvodů (stejný vzor jako ImportLogCard
 *  výš – appka detail vždycky ukazuje inline v tabulce, ne v modálu/tooltipu). */
function AnalyzaSkupinaTlacitko({
  label,
  pocet,
  otevreno,
  onToggle,
  ...
  return (
    <button
      ...
      onClick={onToggle}
      disabled={pocet === 0}
      className={`flex items-center justify-between gap-3 rounded-md border px-3 py-2 text-left text-[12.5px] font-semibold transition-colors ${
        pocet === 0
    ...
    >
      ...
      {pocet > 0 && (
        <span
          className={`text-[10px] text-gray-400 transition-transform ${otevreno ? "rotate-180" : ""}`}
        >
          ▼
        </span>
      )}
...
function ZobrazenoZCelku({ zobrazeno, celkem }: { zobrazeno: number; celkem: number }) {
  if (celkem <= zobrazeno) return null;
  return (
    <p className="mt-1 text-[11px] text-gray-400">
      Zobrazeno prvních {zobrazeno} z {celkem}.
    </p>
  );
}
      ...
      <table className="w-full text-left text-[12px]">
        <thead>
          <tr className="border-b border-gray-200 text-gray-500">
            ...
            <th className="py-1 pr-3 font-semibold">Počet PÚ v plánu</th>
 ...
 * na serveru (viz app/api/analyza-nesparovanych-zprav/route.ts), NE tady v
 * prohlížeči (to appku dřív u podobných hromadných průchodů dokázalo
 * zaseknout, viz i scripts/reprocess-all-revizni-zpravy.ts) – appka jen
 * zavolá endpoint a zobrazí hotový výsledek. Nepočítá se automaticky při
 * načtení appky, jen na vyžádání kliknutím.
 */
  ...
  const spustitAnalyzu = async () => {
    ...
    try {
      ...
      setChyba(
        err instanceof Error
          ? err.message
          : "Analýzu se nepodařilo spustit kvůli neznámé chybě."
      );
      setStav("chyba");
    }
  };
  ...
  return (
    <div className="overflow-hidden rounded-lg bg-white shadow-sm">
      <div className="bg-navy px-[18px] py-2.5 text-[13px] font-bold text-white">
        Analýza nepřiřazených revizních zpráv
      </div>
      <div className="flex flex-col gap-4 px-[18px] py-5">
        <p className="text-[12.5px] text-gray-500">
          Porovná všechny uložené revizní zprávy s aktuálním plánem revizí (podle čísla zařízení) a
          spočítá, kolik z nich se nespáruje jednoznačně na přesně jeden záznam v plánu – a proč.
          Výpočet běží na serveru, appka v prohlížeči nic nepočítá, takže tlačítko appku nezasekne.
        </p>

        <div className="flex flex-wrap items-center gap-3">
          <button
            onClick={spustitAnalyzu}
            disabled={stav === "nacitam"}
          ...
          >
            {stav === "nacitam" ? "Analyzuji…" : "Analýza nepřiřazených revizních zpráv"}
          </button>
          {vysledek && (
            <span className="text-[11px] text-gray-400">Poslední běh: {formatLogCas(vysledek.cas)}</span>
          )}
        </div>

        {stav === "chyba" && (
          <p className="rounded-md bg-red-50 px-3 py-2 text-[12.5px] text-red-600">{chyba}</p>
        )}

        {vysledek && (
          <>
            <div className="rounded-md border border-blue-100 bg-blue-50 px-3 py-2 text-[12.5px] text-blue-700">
              Celkem {vysledek.celkemZprav} revizních zpráv, {vysledek.celkemNesparovanych} z nich se
              nespárovalo jednoznačně na přesně jeden záznam v plánu ({vysledek.celkemPlanu} záznamů).
            </div>

            <div className="flex flex-col gap-2">
              <AnalyzaSkupinaTlacitko
                label="Nejednoznačná shoda (víc PÚ se stejným číslem zařízení)"
                pocet={vysledek.viceShod.pocet}
                otevreno={otevrenaSkupina === "vice_shod"}
                onToggle={() =>
                  setOtevrenaSkupina((s) => (s === "vice_shod" ? null : "vice_shod"))
                }
              />
              ...
              <AnalyzaSkupinaTlacitko
                label="Bez odpovídajícího zařízení v plánu"
                pocet={vysledek.bezShody.pocet}
                otevreno={otevrenaSkupina === "bez_shody"}
                onToggle={() =>
                  setOtevrenaSkupina((s) => (s === "bez_shody" ? null : "bez_shody"))
                }
              />
              ...
              {vysledek.chybiCisloZarizeni.pocet > 0 && (
                <>
                  <AnalyzaSkupinaTlacitko
                    label="Chybí číslo zařízení ve zprávě"
                    pocet={vysledek.chybiCisloZarizeni.pocet}
                    otevreno={otevrenaSkupina === "chybi_cislo"}
                    onToggle={() =>
                      setOtevrenaSkupina((s) => (s === "chybi_cislo" ? null : "chybi_cislo"))
                    }
                  />
                  {otevrenaSkupina === "chybi_cislo" && (
